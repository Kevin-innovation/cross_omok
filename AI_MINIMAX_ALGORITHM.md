...
**왜 중앙(4번째 열)이 중요한가?**
- **7개 열 중 정중앙**: 가로/세로/대각선 **모든 방향**으로 승리 가능
- 가장자리는 승리 경로가 제한적
- 수학적으로 증명: 첫 수를 중앙에 두면 선공 유리
...
**열 번호 체계:**
```
1열  2열  3열  4열  5열  6열  7열  (사람이 세는 방식)
 0    1    2    3    4    5    6   (코드 인덱스)
              ↑
           중앙!
```

- **첫 수는 무조건 중앙 (4번째 열 = 인덱스 3)**
- 중앙 가중치 최대 (13점)
- 가로, 세로, 대각선 **모든 방향**에서 승리 가능
- 수학적으로 증명된 필승 전략의 시작점
...
3. **중앙(4번째 열) 제어 + 포크 전략** (평가 함수)
4. **최적의 수 선택** (수학적으로 증명된 전략)

**중요: 열 번호 표기**
- **사람 표현**: 1~7번째 열 (중앙 = 4번째 열)
- **코드 표현**: 0~6 인덱스 (중앙 = 인덱스 3)
